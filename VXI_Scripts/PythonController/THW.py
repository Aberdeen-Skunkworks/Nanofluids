...
def CvD(T):
    '''The ratio of a platinum resistance to a resistance at 0 degc'''
    A = 3.9083E-3
    B = -5.775E-7
    C = -4.183E-12
    return 1+A*T + B*T**2- 100*C*T**3+ C*T**4

#To predict a wire resistance we use the R=pL/A formula with p being the resistivity. At 20C p=1.06e-7.
def wireR(L, T, D=0.000015):
    p=1.06e-7 #conductivity at 20C
    Awire=math.pi * (D/2)**2 #wire area
    R20 = p * L / Awire
    return R20 / CvD(20) * CvD(T)
